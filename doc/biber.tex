...
\item The \texttt{map\_foreach} attribute loops over all \cmd{step}s in
  ...
  either be the name of a field which contains a comma-separated values
  list or can be an explicit comma-separated values list itself. This
  allows the user to repeat a group of \opt{map\_step}s for each value of
  \textcolor{red}{\texttt{loopval}}.q The special variable
  \verb+$MAPUNIQ+ may also be used the \opt{map\_step}s to generate a
