...
\noindent Or here, mapping journal articles into \bibtype{Report} entries for
...
\noindent Or here, dropping the \bibfield{N2} field from \verb+RIS+
...
\hspace*{2em}\biblatex per-sortscheme \opt{locale} option\\
\hspace*{3em}\biblatex global sortlocale option\\
...
\biblatex passes sorscheme-specific sorting locales and its global sorting
locale in the \file{.bcf}. \biber uses these locales automatically to
tailor each sort list individually. \biblatex can be configured to
...
exactly in your \latex source as the \biblatex \opt{sortlocale} option or
using the \cmd{locale} macro within a \cmd{DeclareSortingScheme} macro. For example,
if you want to use traditional Spanish for sorting a reference list, you
need to specify \verb+es_ES_trad+ directly instead of using the «spanish»
string because the Babel/Polyglossia «spanish» language identifier by
default maps to the modern \verb+es_ES+ locale (which doesn't include sort
tailoring for «ch» in Spanish).
...
\textbf{Note:} \biber\ sets the Unicode collation tailoring
...
UCA default behaviour, see examples.
...
\biber\ takes care of re-encoding the datasource data as
necessary. In normal use, \biblatex\ passes its
\opt{input\_encoding} option value to \biber\ via the \verb+.bcf+
file. It also passes the value of its \opt{texencoding} option (which
maps to \biber's \opt{--output\_encoding|-E} option) the default value
...
\item Decodes \latex\ character macros into UTF-8 if \opt{--output\_encoding|-E}
  is UTF-8
...
\begin{enumerate}
\item When \opt{--output\_encoding|-E} is UTF-8
...
If your \opt{output\_encoding} is not UTF-8, and you are using some UTF-8
equivalent \latex\ character macros in your \verb+.bib+ datasource, then some
...
when \verb+\sortinit{}+ is a char not valid in the \opt{output\_encoding},
...
\item Can detect whether dependencies like datasources have changed without
...
australian & \texttt{en\_US}  & english     & \texttt{en\_US}  & lithuanian      & \texttt{lt\_LT} &   serbian         & \texttt{sr\_Cyrl}\\ 
...
bgreek     & \texttt{el\_GR}  & greek       & \texttt{el\_GR}  & norsk           & \texttt{nn\_NO} &   turkish         & \texttt{tr\_TR}\\
