...
\titlepage{%
  ...
  revision={biber 0.9.4 (biblatex 1.6)},
  date={\today}}
...
The value of the \verb+ignore+ option can only be set in the config
file and not on the command line as it has potentially complex
structure. This config section allows you to specify which fields a
particular data source driver is to ignore when reading the data. This
ignoring is done \emph{before} processing any data source field
aliases, that is, they are specified as the data source specific field
names. For example, to completely ignore the «abstract» field and the
«usera» field in \verb+bibtex+ data sources and also the «N2» field in
\verb+ris+ data sources, put the following in your \verb+biber.conf+:
