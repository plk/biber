...
\item Automatically encode the \verb+.bbl+ into any supported encoding
  format\footnote{«Supported» here means encodings supported by the
    Perl \verb+Encode+ module - \url{http://perldoc.perl.org/Encode.html}}
...
\item On the first run of a new version (that is, with a specific checksum),
  ...
  operating system but is usually something like \verb+/var/tmp+ or \verb+/tmp+ on
  UNIX-based OSes and\\
  \verb+C:\Documents and Settings\<username>\Local Settings\Temp+ on
  Windows. This unpacking can take a little while and only happens on
...
The "\verb+sudo+" commands will prompt you for your password.
...
\noindent Make sure \verb+/usr/local/bin+ is in your PATH. Search Google for "\verb+set PATH
...
You must have the latest CPAN versions of all modules as \verb+biber+
...
follows\footnote{On UNIXequse systems, you may need to specify a full
  path to the scripts e.g. \verb+./Build+}:
...
\item A list of modules/libraries to include in the binary which are not
  automatically detected by the \verb+PAR::Packer+ depenendency
  scanner
...
\noindent To build \verb+biber+ for a new architecture, you should:

\begin{itemize}
\item Make a new subfolder in \verb+dist+ directory named after the
  architecture you are building for. This name is arbitrary but should
  be fairly obvious like «solaris-sparc-64», for example.
