...
\gdef\biberversion{2.0}    % BIBER VERSION
...
\minisec{1.0}
The \opt{--validate\_structure} option is now called \opt{--validate\_datamodel}

\minisec{0.9.9}
The output format option \opt{--graph} has been moved to a
new option \opt{--output\_format}. The option \opt{--graph} should now be
specified as \opt{--output\_format=dot} and the \linebreak\opt{--dot\_include} option
...
cited entries from the datasources (using the \opt{--output\_format=bibtex}
...
\item Support for multiple language/form variants of fields within the same
...
Running \opt{biber --help} will display all options and a brief
...
labelnameform      & & &  \checkmark\\
labelnamelang      & & &  \checkmark\\
labelnamespec      & \checkmark & \checkmark &  \\
labelnumber        & \checkmark & \checkmark &  \\
labeltitle         & \checkmark & \checkmark &  \\
labeltitleform      & & &  \checkmark\\
labeltitlelang      & & &  \checkmark\\
...
vform              & \checkmark &  & \checkmark\\
vlang              & \checkmark &  & \checkmark\\
vtranslang         & \checkmark &  & \checkmark\\
\bottomrule
\end{tabular}
\end{center}
...
\opt{nosort}, \opt{noinit} and \opt{collate\_options} options which are slightly
...
\subsubsection{The \texttt{output\_format} option}\label{ref:of}
...
experimental \verb+biblatexml+ XML format and also \bibtex\ \file{.bib} files.
...
citation order, formatted according to the various \opt{ouput\_*} options.
...
<map +\textcolor{blue}{map\_overwrite="0|1"}+>
  ...
  <map_step +\textcolor{blue}{map\_type\_source="\textcolor{red}{source-entrytype}"}+
             ...
             +\textcolor{blue}{map\_field\_set\_form="\textcolor{red}{set-field-form}"}+
             +\textcolor{blue}{map\_field\_set\_lang="\textcolor{red}{set-field-lang}"}+
             ...
             +\textcolor{blue}{map\_nogeneralise\_variants="1"}+/>
</map>
...
\item Change the \textcolor{red}{\texttt{source-field}} to
  ...
  \textcolor{red}{\texttt{source-field}} field in the entry,
  processing of this \opt{map} immediately terminates.
  \textcolor{red}{\texttt{source-field-form}},
  \textcolor{red}{\texttt{source-field-lang}},
  \textcolor{red}{\texttt{target-field-form}} and
  \textcolor{red}{\texttt{target-field-lang}} can be used to select
  specific variants of sources/targets for a multiscript field. If
  neither are specified, all variants of
  \textcolor{red}{\texttt{source-field}} will be mapped to
  corresponding variants of \textcolor{red}{\texttt{target-field}}
  (unless \textcolor{blue}{\texttt{map\_nogeneralise\_variants}} is
  defined for this step). If
  only \textcolor{red}{\texttt{source-field-form}} is specified, the
  mapping will select the variant of the field with the specified
  form, whatever the language of the field variant. If there is more
  than one field variant with the same
  \textcolor{red}{\texttt{source-field-form}} and different lang, the
  \textcolor{red}{\texttt{source-field}} will not be selected. If only
  \textcolor{red}{\texttt{source-field-lang}} is specified, the
  mapping will select the variant of the field with the specified
  lang, whatever the form of the field variant. If there is more than
  one field variant with the same
  \textcolor{red}{\texttt{source-field-lang}} and different form, the
  \textcolor{red}{\texttt{source-field}} will not be selected.
\item If \textcolor{blue}{\texttt{map\_match}} is defined but
  \textcolor{blue}{\texttt{map\_replace}} is not, only apply the step
  if the \textcolor{red}{\texttt{source-field}} matches
  \textcolor{blue}{\texttt{map\_match}}. You can use parentheses as
  usual to capture parts of the match and can then use these later
  when setting a \textcolor{blue}{\texttt{map\_field\_value}}.
\item \textcolor{blue}{\texttt{map\_notmatch}} is the same as
  \textcolor{blue}{\texttt{map\_match}} but with the logic reversed.
\item Perform a Perl regular expression match and replace on the value
  of \textcolor{red}{\texttt{source-field}} if
  \textcolor{blue}{\texttt{map\_match}} and
  \textcolor{blue}{\texttt{map\_replace}} are defined. If
  \textcolor{red}{\texttt{source-field-form}} and
  \textcolor{red}{\texttt{source-field-lang}} are not specified,
  operates on all variants of \textcolor{red}{\texttt{source-field}}
  unless \textcolor{blue}{\texttt{map\_nogeneralise\_variants}} is
  defined for this step. You may use (and almost certainly will want
  to use) parentheses for back-references in
  \textcolor{blue}{\texttt{map\_replace}}.  Do not quote the regular
  expressions in any special (i.e. non-Perly) way---it's not
  necessary.
\item If \textcolor{blue}{\texttt{map\_field\_set}} is defined, then
  its value is \textcolor{red}{\texttt{set-field}} which will be set
  to a value specified by further
  attributes. \textcolor{red}{\texttt{set-field-form}} and
  \textcolor{red}{\texttt{set-field-lang}} can be specified to target
  a specific field variant. If neither are specified, all variants of
  \textcolor{red}{\texttt{set-field}} will be operated on when
  specifying \textcolor{blue}{\texttt{map\_null}} or
  \textcolor{blue}{\texttt{map\_field\_value}} (unless
  \textcolor{blue}{\texttt{map\_nogeneralise\_variants}} is defined
  for this step). If \textcolor{blue}{\texttt{map\_overwrite}} is
  false for this step and the field to set already exists then the map
  step is ignored. If \textcolor{blue}{\texttt{map\_final}} is also
  set on this step, then processing of the parent map stops at this
  point. If \textcolor{blue}{\texttt{map\_append}} is set, then the
  value to set is appended to the current value of
  \textcolor{red}{\texttt{set-field}}. The value to set is specified
  by a mandatory one and only one of the following attributes:
  \begin{itemize}
...
Note that for XML datasources, the names of fields and entrytypes are
matched in a case sensitive manner. For all other datasource types
(\BibTeX, RIS etc.) entrytype and field name matching is case insensitive.
...
particular name, which presumably has some variation in the datasource.

\begin{lstlisting}[language=xml,escapechar=;,mathescape=true]
<maps>
  <map map_overwrite="1">
    <map_step map_field_source="TITLE" map_field_source_form="translated"
              map_final="1"/>
    <map_step map_null="1"/>
  </map>
  <map map_overwrite="1">
    <map_step map_field_source="SUBTITLE" map_field_source_lang="french"
              map_final="1"/>
    <map_step map_null="1"/>
  </map>
  <map map_overwrite="1">
    <map_step map_field_source="NOTE" map_field_source_form="translated"
              map_field_source_lang="german" map_final="1"/>
    <map_step map_null="1"/>
  </map>
  <map map_overwrite="1">
    <map_step map_field_set="ADDENDUM"/>
    <map_step map_null="1"/>
  </map>
</maps>
\end{lstlisting}

\noindent Removes a translated \bibfield{TITLE} variant without having to
know the language of the variant (the language will be found, assuming that
there is only one translated variant. If there is more than one, this step
will be skipped). Removes a french language variant of the
\bibfield{SUBTITLE} field without having to know the form of the variant
(the form will be found, assuming that there is only one french variant. If
there is more than one, this step will be skipped). Removes any
\bibfield{NOTE} fields with translated form and german
language. Removes all variants of the \bibfield{ADDENDUM} field.
...
\noindent Any entry with a \bibfield{TITLE} field matching «A Title»
will be completely ignored.
\bigskip
\minisec{Other datasource types}
For datasources other than \BibTeX, (e.g. \verb+ris+), the source
entrytypes and fields are usually very differently modelled and named. For
example, here is how to drop the \bibfield{N2} field from \verb+RIS+
...
<inheritance>
  ...
  <inherit>
    ...
    <field source="+\textcolor{red}{source}+"
           +\textcolor{blue}{source\_form="\textcolor{red}{sform}"}+
           +\textcolor{blue}{source\_lang="\textcolor{red}{slang}"}+
           +\textcolor{blue}{target="\textcolor{red}{target}"}+
           +\textcolor{blue}{target\_form="\textcolor{red}{tform}"}+
           +\textcolor{blue}{target\_lang="\textcolor{red}{tlang}"}+
...
\item An \texttt{inherit} element specifies how one or more
  ...
  Specific variants can be specified using the
  \textcolor{blue}{\texttt{source\_form}},
  \textcolor{blue}{\texttt{source\_lang}},
  \textcolor{blue}{\texttt{target\_form}} and
  \textcolor{blue}{\texttt{target\_lang}} optional attributes.
...
\begin{lstlisting}[language=xml,escapechar=+,mathescape=true]
<sorting +\textcolor{blue}{locale=\textcolor{red}{locale}}+>
  ...
  <sort order="+\textcolor{red}{n}+"
        +\textcolor{blue}{locale=\textcolor{red}{locale}}+
    ...
    <sortitem order="+\textcolor{red}{m}+"
              +\textcolor{blue}{form="\textcolor{red}{form}"}+
              +\textcolor{blue}{lang="\textcolor{red}{lang}"}+
...
\textcolor{red}{\texttt{field}} (optionally a particular variant of a field
specified by the \textcolor{blue}{form} and \textcolor{blue}{lang}
attributes, see the \biblatex documentation), you can use the
...
The locale for sorting is specified globally via the \biblatex
\opt{sortlocale} option but this can be overriden on a per-sortscheme or
even per \texttt{sort} element level using the \textcolor{blue}{locale}
attribute. See examples in the \biblatex documentation for
\cmd{DeclareSortingScheme}.
...
\end{lstlisting}
%
Here is a scheme which by default sorts with the «de\_DE» locale. It first looks
for a translated Danish name or a name with no variant specified and sorts
the name found in the «da\_DK» locale. Then it
looks for a translated german title, 
translated Danish title or a title with no variant specified:

\begin{lstlisting}[language=xml,escapechar=+,mathescape=true]
<sorting locale="de_DE">
  <sort locale="da_DK" order="2">
    <sortitem form="translated" lang="danish" order="1">author</sortitem>
    <sortitem order="2">author</sortitem>
  </sort>
  <sort order="2">
    <sortitem form="translated" lang="german" order="1">title</sortitem>
    <sortitem form="translated" lang="danish" order="2">title</sortitem>
    <sortitem order="3">title</sortitem>
  </sort>
</sorting>
\end{lstlisting}
\bigskip
...
The \opt{collate\_options} option has format similar to
...
\hspace*{1em}In the \opt{--output\_directory}, if specified$\rightarrow$\\
...
\opt{--output\_directory|-c}, if this option is defined. You can
...
will not be able to write the \verb+.bbl+. Use the \opt{--output\_file|-O}
...
\biberex{--collate|-C}
  ...
  UCA. It is a little slower than \opt{--fastsort|-f} but the
...
\noindent\opt{--collate\_options|-c} (e.g. \opt{-c 'locale => "de\_DE"'}) $\rightarrow$\\
...
\noindent With the default \opt{--collate|-C} option, the locale will be
...
With the \opt{--fastsort|-f} option, the locale will be
...
\opt{--collate|-C} by default collates uppercase before lower.
...
override it but \biber\ will warn you if you do. \opt{sortupper} has
no effect when using \opt{--fastsort|-f}--you are at the mercy of what
your OS locale does.
...
Options are passed using the \opt{--collate\_options|-c} option as a
...
\biberex{biber --collate_options="variable => 'shifted'"}
...
\opt{--input\_encoding|e} option. \biblatex also passes the value of its
\opt{texencoding} option (which maps to \biber's
\opt{--output\_encoding|-E} option) the default value of which depends on
...
\item Encodes the output so that the \verb+.bbl+ is in
  the encoding that \opt{--output\_encoding|-E} specifies
\item Warns if it is asked to output to the \verb+.bbl+ any UTF-8
  decoded \latex\ character macros which are not in the
  \opt{--output\_encoding|-E} encoding. Replaces with a suitable \latex\ macro
\end{enumerate}
...
\noindent\opt{--input\_encoding|-e} and \opt{--output\_encoding|-E} $\rightarrow$\\
...
user selects \opt{--output\_encoding=ascii} etc. This means that the many
...
set as a value for \opt{--decodecharsset} or \opt{output\_safecharsset}.
...
\opt{--output\_safechars} option which will try to convert any UTF-8 chars into
...
You can force this with the \opt{--output\_safechars} option which will do a
...
(which corresponds to the \biber\ option «\opt{--output\_encoding|-E}») is
set to an ASCII encoding («\verb+ascii+» or «\verb+x-ascii+») and
«\opt{--input\_encoding|-e}» is not ASCII, \biber\ will automatically set
\opt{--output\_safechars}.

Since \biber always decodes into UTF-8 internally, if the
\opt{--output\_encoding|-E} option is not set to UTF-8, \biber will
...
See also the \opt{biber --help} output for the \opt{--output\_safecharsset}
...
recoding sets for use with \opt{--output\_safecharsset} and
\opt{--decodecharsset}.
...
mapping from UTF-8, just in case there are more than one mappings from
...
\noindent Set \opt{input\_encoding} and \opt{output\_encoding} from the
config file or \verb+.bcf+

\biberex{biber --output_encoding=latin2}

\noindent Encode the \verb+.bbl+ as latin2, overriding the
\verb+.bcf+

\biberex{biber --output_safechars}

\noindent Set \opt{input\_encoding} and \opt{output\_encoding} from the
config file or \verb+.bcf+. Force encoding of UTF-8 chars to \latex\
macros using default conversion set

\biberex{biber --output_encoding=ascii}

\noindent Encode the \verb+.bbl+ as ascii, overriding the
\verb+.bcf+. Automatically sets \opt{--output\_safechars} to force UTF-8
to \latex\ macro conversion

\biberex{biber --output_encoding=ascii --output_safecharsset=full}
...
\noindent Shortcut alias for \opt{biber --input\_encoding=UTF-8}

\biberex{biber -U}

\noindent Shortcut alias for \opt{biber --output\_encoding=UTF-8}
...
\opt{--namesep=und} and \opt{--others\_string=andere} and then you can do:
\begin{verbatim}
@BOOK{key,
...
The option \opt{--output\_format=dot} will cause \biber\ to write a
...
\noindent The \opt{--dot\_include} option takes a comma delimited string as
...
\ref{tab:graphopts}. If the \opt{--dot\_include} option is not given
...
the options \opt{output\_file} and \opt{output\_format} are not specified.

Tool mode is useful if you need to programatically change your
datasource using the semantics provided by \biber or if you would like
to convert your data to a different format. For example, you
could choose to reencode your datasource by turning all UTF-8
characters into \latex\ macros:
...
\opt{--output\_safechars} option is automatically enabled). If you utilise the \biber\
...
\opt{--output\_resolve} option which will process any \bibfield{XDATA}
...
\opt{--output\_macro\_fields} option to specify a comma-separated list
...
\opt{--tool\_indent} can be used to customise the indentation of
...
\end{lstlisting}
%
Further suppose that the \file{biber-test.conf} contains the following:
...
\opt{--tool\_resolve} option, inheritance processing is performed on the
...
\midrule
output\_align & \checkmark & - & - & -\\
output\_indent & \checkmark & \checkmark & - & -\\
output\_fieldcase & \checkmark & - & - & -\\
output\_macro\_fields & \checkmark & - & - & -\\
output\_resolve & \checkmark & \checkmark & - & -\\
\bottomrule
\end{tabular}
\end{center}
...
tool mode config file by using the \opt{--tool\_config} option which will
...
«\texttt{en\_US}» if you do not use \biber's \opt{sortlocale} option.
\bigskip%|<- stop confusing emacs highlighting
\subsection{Extended \bibtex\ Datasource Field Format}
\biber\ implements an extended \bibtex\ datasource format which allows
users to specify the script form and language for a field. In this
way, it is possible to include several variants of a field in an entry
which give the field in different script forms (translated, romanised
etc.) with an option in cases of forms like translated forms, to
specify the language. This allows users within \biblatex\ to access
such data when formatting entries. The extended field format looks
like this:
...
\end{verbatim}
%
Both \texttt{<form>} and \texttt{<lang>} are optional and if they are not
present, default to the values of the \biblatex options \opt{vform} and
\opt{vlang} respectively. \texttt{<lang>},
if specified, should be a known \sty{babel}/\sty{polyglossia} language
identifier. \texttt{<form>} can be one of:
\begin{itemize}
\item original (default if ommitted)
\item translated
\item romanised
\item uniform
\end{itemize}
...
@BOOK{example1,
  ...
  AUTHOR_translated_english = { Bulgakov, Pavel and Rosenfeld, Boris },
  TITLE                     = { Славься, Отечество наше свободное },
  TITLE_romanised           = { Slavʹsâ, Otečestvo naše svobodnoe },
}
\end{lstlisting}
%
This example has an \bibfield{AUTHOR} field in Russian Cyrillic script
and a variant the «translated» form with and language setting of
«english». There is also a Russian Cyrillic \bibfield{TITLE} with an
ISO 9 romanised variant. \biblatex has been modified so that all data
accessing and comparison macros have an option keyval argument which can be
used to select a particular form/lang variant, if desired. The separator
between the field, form and lang can be customised with the \biber
\opt{vsplit} option and defaults to an underscore (ASCII 95).
\bigskip
...
\noindent The \opt{--link} options can be a little tricky
...
\opt{--link} option. The \opt{--module} options are the same for all
