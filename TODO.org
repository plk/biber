...
* TODO LabelalphaNameTemplate implementation
** DONE .bcf read
** DONE config.pm opt read
** DONE compound, strwidth replace pstrwidth, pcompound and compound
** DONE xsl update
** TODO Doc - move pstrwidth (remove) -> strwidth, pcompound (remove) -> compound, strwidth on lant overrides on lat
* TODO sortgiveninits
** TODO Should be moved to option of \namepart in SNKS
** TODO Remove biber option and make biblatex deprecated
** TODO Change schema, .bcfs, xsl, doc
** TODO Fix in Internals, Remove from Constants.pm and Config.pm
* TODO 'given' in _getnamehash_u in Internals - can be abstracted?
...
* TODO 5.2.2 EDTF? (see github comments).
