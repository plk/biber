...
\titlepage{%
  ...
  revision={biber 0.8.3 (biblatex 1.2b)},
  date={\today}}
...
\subsection{Biber datasource drivers}
...
without know what the named driver handlers do specifically but there
...
suck as much meaningful information from a data source as possible.
...
Binaries are available for many architectures. These are the core
architectures which are developed for and which are regularly updated on
SourceForge:
...
\item \verb+cygwin+ --- Cygwin. Built with 1.7.8, probably won't  work on $<$ 1.7
\item \verb+darwin_x86+ --- OSX Intel 32/64-bit universal (built on OSX 10.6)
\end{itemize}

\noindent In addition, \verb+biber+ is included in TeXLive which provides
binaries for more architectures than I am able to provide regularly.
The downside is that these other architectures probably won't be quite so
up to date but they shouldn't be that far behind. If you want to run
development versions, you'll either have to run one of the core supported
architectures or use the perl source version.

\noindent Binaries for the following platforms are in TeXLive and are also
available directly from the following locations:

\begin{itemize}
\item \verb+FreeBSD+ --- \url{http://anthesphoria.net/FreeBSD/Biber/}
\end{itemize}
...
which preferably doesn't have perl at all on it. Running the binary with no
arguments will unpack it in the background and display the help. To really
test it, run LaTeX (using \verb+biblatex+) on a simple test file named,
say, «\verb+test.tex+» and then run

\begin{verbatim}
biber test --convert_control
\end{verbatim}

\noindent This option will run an XSLT transform on the \verb+.bcf+ and
leave an HTML representation of it in the same directory as the
\verb+.bcf+. This is a good test to make sure that the linking of the XML
and XSLT etc. libraries worked which is one of the most tricky parts of the
build process.
\end{document}
