...
\noindent As first example but override locale.

\biberex{\verb+biber --fastsort+}

\noindent As first example but use fastsort. This uses the OS locale
files which may or may not be accurate.

\biberex{\verb+biber --sortupper=0 --collate_options="backwards => 2"+}

\noindent As first example but collate lowercase before upper and
collate French accents in reverse order at UCA level 2.
