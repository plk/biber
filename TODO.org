...
* TODO When dates are all DT, sorting must be numeric on ->epoc due to single-digit returns from DT methods. Put in new cmp operator spec in DM or generate from DM types.
