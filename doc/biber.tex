...
\begin{figure}[!htpb]
  ...
  \begin{tikzpicture}
    ...
    \node[entity] (dsfile) {Datasource};
    \node[isa] (remap) [below of=dsfile, node distance=8em] {remap} edge [<-,dashed] (dsfile);
    \node[entity] (conf) [below of=remap,  node distance=7em] {biber config file} edge [->,dashed] (remap);
    \node[isa] (parser) [right of=remap, node distance=8em] {parser} edge [<-,very thick] (remap);
    \node[isa] (validation) [right of=parser, node distance=8em] {validation} edge [<-,very thick] (parser);
    \node[entity] (bcf) [above of=parser, node distance=8em] {.bcf control file} edge [->,dashed] (parser) edge [->,dashed] (validation);
    \node[entity] (hard) [below of=parser, node distance=12em] {static data model} edge [->,dashed] (parser) edge [->,dashed] (validation);
    \node[entity] (bbl) [right of=parser, node distance=16em] {.bbl file} edge [<-,very thick] (validation);
