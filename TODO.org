...
** TODO compound, strwidth replace pstrwidth, pcompound and compound
** TODO By default, prefix not base so it's appended in order before base regardless of base disambiguations which come later (which is the case now).
** TODO xsl update
** TODO doc new opt

* TODO sortgiveninits check in Internals - can be abstracted?
