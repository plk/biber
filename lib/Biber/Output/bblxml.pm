...
sub set_output_target_file {
  ...
  my $schemafile = $bblxmlfile =~ s/\.(?:$exts)$/.rng/r;

  my $of = IO::File->new($bblxmlfile, '>:encoding(UTF-8)');
  ...
  $xml->xmlDecl();
  $xml->pi('xml-model', "href=\"$schemafile\" type=\"application/relax-ng-compact-syntax\"");
...
sub set_output_entry {
  ...
  foreach my $namefield (@{$dm->get_fields_of_type('list', 'name')}) {
    ...
    if ( my $nf = $be->get_field($namefield) ) {
      ...
      $xml->startTag([$xml_prefix, 'names'], type => $namefield, count => $total, %plo);
  ...
  foreach my $listfield (@{$dm->get_fields_of_fieldtype('list')}) {
    ...
    if (my $lf = $be->get_field($listfield)) {
      ...
      my $total = $#$lf + 1;
      $xml->startTag([$xml_prefix, 'list'], type => $listfield, count => $total, %plo);
      foreach my $f (@$lf) {
  ...
  $xml->dataElement([$xml_prefix, 'namehash'], NFC($namehash)) if $namehash;
  my $fullhash = $be->get_field('fullhash');
  $xml->dataElement([$xml_prefix, 'fullhash'], NFC($fullhash)) if $fullhash;

  if ( Biber::Config->getblxoption('labelalpha', $bee) ) {
    # Might not have been set due to skiplab/dataonly
    if (my $label = $be->get_field('labelalpha')) {
      $xml->dataElement([$xml_prefix, 'labelalpha'], NFC($label));
    }
  }
  ...
  if ( Biber::Config->getblxoption('labeldate', $bee)) {
    ...
    if (my $ly = $be->get_field('labelyear')) {
      $xml->dataElement([$xml_prefix, 'labelyear'], NFC($ly));
    }
    if (my $lm = $be->get_field('labelmonth')) {
      $xml->dataElement([$xml_prefix, 'labelmonth'], NFC($lm));
    }
    if (my $ld = $be->get_field('labelday')) {
      $xml->dataElement([$xml_prefix, 'labelday'], NFC($ld));
    }
    if ($be->field_exists('datelabelsource')) {
      $xml->dataElement([$xml_prefix, 'datelabelsource'], NFC($be->get_field('datelabelsource')));
    }
  }
  ...
  if (my $lni = $be->get_labelname_info) {
    $xml->dataElement([$xml_prefix, 'labelnamesource'], NFC($lni));
  }

  # The source field for labeltitle
  if (my $lti = $be->get_labeltitle_info) {
    $xml->dataElement([$xml_prefix, 'labeltitlesource'], NFC($lti));
  }

  if (my $ck = $be->get_field('clonesourcekey')) {
    $xml->dataElement([$xml_prefix, 'clonesourcekey'], NFC($ck));
  }
  ...
  foreach my $rfield (@{$dm->get_fields_of_datatype('range')}) {
    ...
    if ( my $rf = $be->get_field($rfield) ) {
      ...
      my @pr;
      $xml->startTag([$xml_prefix, 'field'], name => $rfield);
      foreach my $f (@$rf) {
  ...
  foreach my $uril (@{$dm->get_fields_of_type('list', 'uri')}) {
    ...
    if ( my $urilf = $be->get_field($uril) ) {
      ...
      my $total = $#$urilf + 1;
      $xml->startTag([$xml_prefix, 'list'], name => $uril, count => $total, %plo);
...
sub output {
  ...
  my $exts = join('|', values %DS_EXTENSIONS);
  my $schemafile = $target_string =~ s/\.(?:$exts)$/.rng/r;
