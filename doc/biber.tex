...
identifier. The \texttt{<form>} must be one of the forms declared by the
\biblatex macro \cmd{DeclareVariantforms}. The default is:
\begin{itemize}
\item original (default if omitted)
\item translated
\item romanised
\item uniform
\end{itemize}
...
\end{lstlisting}
%
This example has an \bibfield{AUTHOR} field in Russian Cyrillic script and
a variant the «uniform» form. There is also a Russian Cyrillic
\bibfield{TITLE} with an ISO 9 romanised variant. \biblatex has been
modified so that all data accessing and comparison macros have an option
keyval argument which can be used to select a particular form/lang variant,
if desired. The separator between the field, form and lang can be
customised with the \biber \opt{vsplit} option and defaults to an
underscore (ASCII 95).
\bigskip
