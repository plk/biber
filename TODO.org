...
* TODO sorting scheme names are not used. Create multiple and refer by name as with other dlist
* TODO presort, exclude and include should not be inside a sorting spec

\blx@collectopts{xml}{blx@globalopts}{sortscheme}% for passing to the .bcf
sort/sortscheme are wrong datatypes in .bcf
