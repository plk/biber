...
\item If \textcolor{blue}{\texttt{map\_entry\_clone}} is set, a clone of
  the entry is created with an entry key prefixed by
  \textcolor{red}{\texttt{prefix}}. Obviously this may cause labelling
  problems in author/year styles etc. and should be used with care. It is
  mainly intended for numbering issues with numeric styles and multiple
  bibliographies where the same entries occur in more than one
  bibliography. The cloned entry in-scope during the processing of the
