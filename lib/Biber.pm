...
sub parse_auxfiles {
    ...
    while (<$aux>) {
        ...
        if ( ! -f $biberfile and $_ =~ /^\\bibdata/ ) { 
            ...
            $self->{config}->{bibdata} = [ push @bibdatafiles, @tmp ] ;
        }
    ...
    $self->parse_biberfile($biberfile) if $biberfile ;
...
#    my @auxcitekeys = $self->citekeys ;
    ...
    croak "Cannot find file '$biberfile'!\n" unless -f $biberfile ;
    croak "File '$biberfile' is not a .biber file!\n" unless $biberfile =~ m/\.biber$/ ;

    my $biberaux = new IO::File "<$biberfile" or croak "Failed to open $biberfile : $!" ;

    local $/ = "\n" ;

    print "Reading $biberfile\n" unless $self->config('quiet') ;
    ...
    while (<$biberaux>) {
		...
		if ( /^\s*([^=]+?)\s*=\s*(.*?)\s*$/ ) {
            if ( $1 eq 'bibdata' ) {
                my @tmp = split/,/, $2 ; 
                $self->{config}->{bibdata} = [ @bibdatafiles, @tmp ] ;
            } else {
                $self->{config}->{$1} = $2;
            }
			next;
        ...
        #TODO handle sections "[citekeys.section1]"
        #
#        if ( /^cite:/ ) { 
#            m/^cite:(\S+)/ ;  
#            if ( $1 eq '*' ) {
...
#                    unless ( $self->config('quiet') ) ;
#
#                # we stop reading the aux file as soon as we encounter cite:*
#                last
#
#            } elsif ( ! $seenkeys{$1} ) {
#
...
#        }
        ...
        ### return $self->error( "Syntax error at line $. : '$_'" );

    }

#    unless ($self->config('allentries') or @auxcitekeys) {
#        croak "The file '$biberfile' does not contain any citations!\n"
#    }
#
#    print "Found ", $#auxcitekeys+1 , " citekeys in .biber file\n" 
#        unless ( $self->config('quiet') or $self->config('allentries') ) ;
#
#    @auxcitekeys = sort @auxcitekeys if $self->config('biberdebug') ;
#
...
sub process_crossrefs {
    ...
    foreach my $k ( keys %crossrefkeys ) {
        if ( ! $seenkeys{$k} and $crossrefkeys{$k} >= $self->config('mincrossrefs') ) {
