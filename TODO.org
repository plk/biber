...
*** TODO msform/mslang biblatex options and LANGID override below ENTRY scope. The options control which alternates are selected in various templates by biber.
*** TODO msform/mslang \printbibliography/\printbiblist options to select the defaults for \printfield etc. Don't need to be refcontext options as mslang/msform are already addressed as they indirectly change the current refcontext templates which in turn changes data.
*** TODO biber mslang option and that this is checked after biblatex global/entry (for tool mode use)
** TODO biblatex
*** DONE Datamodel
...
*** TODO msform and mslang as biblatex options with scope down to ENTRY
